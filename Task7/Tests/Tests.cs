...
#region Settings
        ...
        public void Test_Cookeis()
        {
            ...
            try
            {
                ...
                browser.Cookie.Add( cookieData.First.Key, cookieData.First.Value);
                browser.Cookie.Add(cookieData.Second.Key, cookieData.Second.Value);
                browser.Cookie.Add(cookieData.Third.Key, cookieData.Third.Value);

                var cookies = browser.Cookie.AsReadonly();
                ...
                browser.Cookie.Delete(cookieData.First.Key);
                cookies = browser.Cookie.AsReadonly();
                Assert.False(cookies.Any(x=>x.Name == cookieData.First.Key));

                loggers.Log(LogType.Info, "Update third cookie", method, 3);
                browser.Cookie[cookieData.Third.Key] = new OpenQA.Selenium.Cookie(cookieData.Third.Key, cookieData.NewCookie3Value);
                Assert.AreEqual(cookieData.NewCookie3Value, browser.Cookie[cookieData.Third.Key].Value);

                loggers.Log(LogType.Info, "Clear cookie", method, 4);
                browser.Cookie.Clear();
                Assert.AreEqual(browser.Cookie.AsReadonly().Count, 0);
